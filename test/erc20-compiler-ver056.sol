...
contract Context {
    function _msgSender() internal view  returns (address) { // virtual
        return msg.sender;
    }
/*    function _msgData() internal view  returns (bytes memory calldata) { // virtual
        return msg.data;
    } */
}
 contract Ownable is Context {
    ...
    function owner() public view  returns (address) { // virtual
        return _owner;
    }
    modifier onlyOwner() {
        require(owner() == _msgSender(), "ERR@ownable: caller is not the owner");
        _;
    }
    ...
    function transferOwnership(address newOwner) public  onlyOwner { // virtual
        require(newOwner != address(0), "ERR@ownable: new owner is the zero address");
...
contract KIP7AKG is Context, IKIP7 , Ownable {
...
//		}
	...
	function query_withdrawable_basispoint ( address _address , uint _querytimepoint ) public view returns (uint ){ //			 getYear(uint timestamp) external returns (uint16);	//	function getMonth(uint timestamp) external returns (uint8);		// function getDay(uint timestamp) external returns (uint8);
		Timelock_taperdown memory timelock_taperdown = _timelock_taperdown[_address ] ;
		if(timelock_taperdown.active) {
			...
			else {} //			uint querytimepoint_year = uint ( ICalendarLibrary( _calendar_lib ).getYear ( _querytimepoint ) ); // ???//			uint querytimepoint_month= uint ( ICalendarLibrary( _calendar_lib ).getMonth( _querytimepoint ) ) ; // ???//			uint querytimepoint_day	 = uint ( ICalendarLibrary( _calendar_lib ).getDay( _querytimepoint ) ) ;		 // ???
...
//////// ???
    ...
    function name() public view  returns (string memory) { // virtual override
        return _name;
    }
  ...
  function symbol() public view  returns (string memory) { //virtual override 
        return _symbol;
    }
  function set_symbol(string memory __symbol ) public {
    ...
    function decimals() public view   returns (uint8) { // virtual override
        return 18;
    }

    function totalSupply() public view returns (uint256) { //virtual   override
        return _totalSupply;
    }

    function balanceOf(address account) public view returns (uint256) { // virtual  override
        return _balances[account];
    }

    function transfer(address recipient, uint256 amount) public returns (bool) { // virtual  override
...
//				require(meets_timelock_terms(msg.sender) , "ERR(72485) time locked" );
    ...
    function allowance(address owner, address spender) public view   returns (uint256) { //virtual override
        return _allowances[owner][spender];
    }

    function approve(address spender, uint256 amount) public   returns (bool) { //virtual override
    ...
    function transferFrom (
			...
			address recipient,
			uint256 amount
    ) public   returns (bool) { // overridevirtual
    ...
    function increaseAllowance(address spender, uint256 addedValue) public  returns (bool) { // virtual
        _approve(_msgSender(), spender, _allowances[_msgSender()][spender] + addedValue);
        return true;
    }

    function decreaseAllowance(address spender, uint256 subtractedValue) public  returns (bool) { // virtual
